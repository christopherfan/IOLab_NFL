...
def readFFTeamsStats():
	json_file = open ('static/Data/ffTeamsStatistics_final.json', 'r')
...
def readTeamSeasonStat():
	json_file = open ('static/Data/teamSeasonStatistics_final.json', 'r')
	json_data = json.load(json_file)
	season_json = {}
...
def readFreeAgentsStats():
	json_file = open ('static/Data/freeagentsStatistics_final.json', 'r')
	json_data = json.load(json_file)
	season_json = {}			
	json_file.close()
	return json_data			
...
def generateRecommendation(ff_team, week, nfl):
	...
	for key_player, value_player in ff_team.iteritems():		
		if value_player['position'] != 'DEF':			
			...
			scoreList.append((value_player['player_name'],value_player['position'], score))						
		else:
