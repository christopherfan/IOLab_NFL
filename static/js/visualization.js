...
function playerSelection(player1,player2)
{
	...
	d3.json("static/Data/ffTeamsStatistics_FreeAgents_Final.json",function(error,json)
	{
		...
		playerARating = player1[2];
		playerBRating = player2[2];
		if (player1[0] == 'team')
		{
			playerA = json['Wait for it'][player1[1]];
		}
		else
		{
			playerA = json[player1[0]][player1[1]];
		}

		if (player2[0] == 'team')
		{
			playerB = json['Wait for it'][player2[1]];
		}		
		else
		{
			playerB = json[player2[0]][player2[1]];
			playerB.player_name = playerB.player_name.charAt(0)+ ". "+playerB.player_name.split(" ")[1];
		}
...
function playerComparison()
{
	...
	$("#playerA").css("background-image","url(static/Player_Images/"+playerA.player_name.split(" ").join("").split(".").join("")+".jpg)")	
	...
	$("#playerA div.recomendation span.scoring").text(playerARating);
	


	//$("#playerB img").attr('src',"img/"+playerB.player_name.split(" ").join("").split(".").join("")+".jpg");
	$("#playerB").css("background-image","url(static/Player_Images/"+playerB.player_name.split(" ").join("").split(".").join("")+".jpg)");
	...
	$("#playerB div.recomendation span.scoring").text(playerBRating);
